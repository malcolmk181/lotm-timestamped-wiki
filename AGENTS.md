...
## Linting & formatting
- `ruff` is configured in `pyproject.toml`: isort (`I`), pyflakes/pycodestyle
  (`F`/`E`/`W`), bugbear (`B`), pyupgrade (`UP`), pylint (`PL`). Line length is
  88, owned by `ruff format` (black-compatible), so `E501` is ignored.
- After every set of edits, run both, then the tests — commit only clean
  output:
  ```sh
